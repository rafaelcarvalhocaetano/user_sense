...
// CloudwatchCore is a zap Core for dispatching webhook to the specified
...
func (c *CloudwatchCore) cloudWatchInit() error {
	s, _ := session.NewSession(c.AWSConfig)
	c.svc = cloudwatchlogs.New(s)
	...
	if len(lgresp.LogGroups) < 1 {
		// we need to create this log applications
		_, er := c.svc.CreateLogGroup(&cloudwatchlogs.CreateLogGroupInput{LogGroupName: aws.String(c.GroupName)})
		if er != nil {
			return er
		}
	}
